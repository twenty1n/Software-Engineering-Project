...
export default function ProviderEditProfile({navigation}) {
  ...
  // Fetch the access token from AsyncStorage on component mount
  useEffect(() => {
  ...
  const handleSave = () => {
    if (isDisabled) {
      ...
      fetch('http://192.168.1.50:5000/provider/update-profile', {
        method: 'PATCH',
        headers: {
  ...
  useEffect(() => {
    ...
    fetch('http://192.168.1.50:5000/provider/api/profile', {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    })
      ...
      .then(results => {
        ...
        setOldPassword(results.provider.provider_password);
      })
      .catch(error => {
        console.error('Profile Fetch Error:', error);
        setError(error.message);
      });
  }, [accessToken]); // Re-fetch when the access token changes
